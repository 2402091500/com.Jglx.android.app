...
public class RegisterActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		register_nextBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (register_YZCodeEdt.getText().toString().equals(code)) {
	...
	public void zhuceCode() {
		RequstClient.getRegeditCode(phone, code, new CustomResponseHandler(
				this, true) {
			@Override
	...
	public void WJPasswordCode() {
		if (LXApplication.wjCount==5) {
			Toast.makeText(this, "今日忘记密码请求已超过次数, 明日再来吧!",Toast.LENGTH_SHORT).show();
			return;
		}
		LXApplication.wjCount=LXApplication.wjCount+1;
		
		RequstClient.getPasswordCode(phone, code, new CustomResponseHandler(
				this, true) {
			@Override
			public void onFailure(String error, String errorMessage) {
				super.onFailure(error, errorMessage);
	...
	public void go_Regedit() {
		RequstClient.doRegedit(phone, pw, "2", new CustomResponseHandler(this,
				true) {
	 ...
	 */
	public void wjPwd() {
		RequstClient.wangJPwd(phone, pw, new CustomResponseHandler(this, true) {
