...
import com.example.com.jglx.android.app.task.LoadImageTask;
import com.example.com.jglx.android.app.task.LoadVideoImageTask;
import com.example.com.jglx.android.app.ui.AlertDialogActivity;
import com.example.com.jglx.android.app.ui.BaiduMapActivity;
import com.example.com.jglx.android.app.ui.ChatActivity;
import com.example.com.jglx.android.app.ui.ContextMenuACtivity;
import com.example.com.jglx.android.app.ui.ShowBigImageActivity;
import com.example.com.jglx.android.app.ui.ShowVideoActivity;
...
public class ChatAdapter extends BaseAdapter {
	...
	private String otherAvatar;// 别人的头像
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			...
			else if (((LXHXSDKHelper) HXSDKHelper.getInstance())
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (TextUtils.isEmpty(otherAvatar)) {
			...
			getUserInfoByChatID(message, holder.iv_avatar, null);
		} else {
		...
		if ((chatType == ChatType.GroupChat)
			...
			getUserInfoByChatID(message, holder.iv_avatar, holder.tv_usernick);
		}
...
//			}
	 ...
	 * @param iv_avatar
	 */
	private void setUserAvatar(EMMessage message, CircleImageView iv_avatar) {

		if (message.direct == Direct.SEND) {
			// 显示自己头像
			if (!TextUtils.isEmpty(LXApplication.mUserInfo.Logo)) {
				iv_avatar.setUrl(LXApplication.mUserInfo.Logo);
			} else {
	...
	private void handleTextMessage(EMMessage message, ViewHolder holder,
		...
		holder.tv.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				activity.startActivityForResult(
						(new Intent(activity, ContextMenuACtivity.class))
								.putExtra("position", position).putExtra(
										"type", EMMessage.Type.TXT.ordinal()),
		...
		holder.iv.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				activity.startActivityForResult(
						(new Intent(activity, ContextMenuACtivity.class))
								.putExtra("position", position).putExtra(
										"type", EMMessage.Type.IMAGE.ordinal()),
						ChatActivity.REQUEST_CODE_CONTEXT_MENU);
				return true;
			}
		});
					...
					activity.runOnUiThread(new Runnable() {
						public void run() {
							...
							if (message.status == EMMessage.Status.SUCCESS) {
								...
								Toast.makeText(
										activity,
										"Send failed ||  connect_failuer_toast",
										0).show();
								timer.cancel();
							}
		...
		holder.iv.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				activity.startActivityForResult(
						new Intent(activity, ContextMenuACtivity.class)
								.putExtra("position", position).putExtra(
										"type", EMMessage.Type.VIDEO.ordinal()),
						ChatActivity.REQUEST_CODE_CONTEXT_MENU);
				return true;
			}
		});
		...
		holder.iv.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				activity.startActivityForResult(
						(new Intent(activity, ContextMenuACtivity.class))
								.putExtra("position", position).putExtra(
										"type", EMMessage.Type.VOICE.ordinal()),
						ChatActivity.REQUEST_CODE_CONTEXT_MENU);
				return true;
			}
		});
		...
		holder.ll_container.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (file != null && file.exists()) {
					...
					// 下载
					Log.e("messageAdapter", "messageAdapter_下载");
			...
			public void run() {
				...
				if (message.status == EMMessage.Status.SUCCESS) {
					...
					if (message.getError() == EMError.MESSAGE_SEND_INVALID_CONTENT) {
						Toast.makeText(activity,
								"send_fail--connect_failuer_toast", 0).show();
	 ...
	 * @param iconIv
	 */
	private void getUserInfoByChatID(final EMMessage mssage,
			final CircleImageView iconIv, final TextView tv) {

		RequstClient.Get_UserInfo_byChatID(mssage.getFrom(),
				new AsyncHttpResponseHandler() {
					@Override
					public void onFailure(Throwable error, String content) {
						super.onFailure(error, content);
						setUserAvatar(mssage, iconIv);
					}
